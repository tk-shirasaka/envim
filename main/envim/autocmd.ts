...
export class Autocmd {
  static setup() {
    ...
    Emit.share("envim:command", "  autocmd BufRead,BufEnter,BufWritePost *.mp4,*.mpeg,*.wmv,*.mov,*.avi call EnvimConnect(0, 'envim_preview', win_getid(), 'video', blob2list(readblob(expand('%'))), expand('%:e'))");
    ...
    Emit.share("envim:command", "  autocmd BufHidden,BufDelete *.ico,*.png,*.jpg,*.jpeg,*.gif,*.svg,*.mp4*,.mpeg,*.wmv,*.mov,*.avi,*.pdf call EnvimConnect(0, 'envim_preview', win_getid())");
